...
public class LabirintoBuilderTest {
	...
	public void testMonolocaleConAttrezzoSingoloDuplicato() {
		...
		assertTrue(size==1);
		assertEquals(lista,monolocale.getStanzaIniziale().getAttrezzi());
	}
	...
	public void testPiuDiQuattroAdiacenti() {
		...
		assertTrue(maze.getStanzaIniziale().getStanze().size()<=4);
		assertTrue(!maze.getStanzaIniziale().getStanze().containsValue(test));
		...
		assertEquals(mappa.keySet(),maze.getStanzaIniziale().getStanze().keySet());
	}
	...
	public void testLabirintoCompletoConTutteLeStanze() {
		...
		for(String d: mapAdiacenti.keySet()) {
			direzioni.add(d);
		}
		assertEquals(direzioni,corridoio.getDirezioni());
